// Больше не используется витриной — с переключением на бэкенд (см. lib/api.ts)
// категории и товары приходят из реального API (GET /catalog/..., раздел 6 плана
// архитектуры). Файл оставлен как справочная копия исходных тестовых данных
// (на них же основан backend/prisma/seed.ts) — можно удалить, когда не понадобится.
